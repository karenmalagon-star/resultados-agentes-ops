...
Deno.serve(async(req)=>{const wk=req.headers.get("x-write-key")||"";const stored=await cfg("write_key");if(!stored)return json({error:"config no disponible (transitorio)"},503);if(wk!==stored)return json({error:"unauthorized"},401);try{
 ...
 // delayAll default: estructura [promedio, n, b0..b5] = 8 elementos (NO son 8 buckets; no "corregir" a 6)
 ...
 return json({ok:!truncado,truncado,eventos:events.length,dias:dates.length,callcube:callCube.length,agentes:agents.length,tiendas:stores.length,desde:data.meta.start,hasta:data.meta.end,cierre_dias:cgen.length});
