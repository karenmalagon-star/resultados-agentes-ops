-- ============================================================================
-- Módulo de Variables · 009 · Filtro de fechas en el Resumen (rango dentro del mes) + detalle por tienda de UN agente
--  · var_resumen_mes(p_mes, p_dia, p_desde, p_hasta): sin p_desde/p_hasta se comporta igual que antes (acumulado del mes);
--    el motor de dinero sigue llamándola con solo p_mes.
--  · var_tiendas_agentes(p_desde, p_hasta, p_agent_id): con agente devuelve solo ese (pop-up de detalle por tienda).
-- ============================================================================
drop function if exists public.var_resumen_mes(date, date);
...
drop function if exists public.var_tiendas_agentes(date, date);
create or replace function public.var_tiendas_agentes(p_desde date, p_hasta date, p_agent_id text default null) returns jsonb
language sql stable set search_path = public, pg_temp as $$
  with ev as (
    ...
    where e.ev_date between p_desde and p_hasta and not var_es_excluido(e.agent) and (p_agent_id is null or al.agent_id = p_agent_id)
  ),
  ...
  select jsonb_build_object('desde', p_desde, 'hasta', p_hasta,
    'agentes', coalesce((select jsonb_agg(jsonb_build_object('agent_id', g.agent_id, 'nombre', g.nombre, 'cargo_permanente', g.cargo_permanente,
      ...
      where (p_agent_id is null and (g.activo or a.agent_id is not null)) or g.agent_id = p_agent_id), '[]'::jsonb))
$$;
