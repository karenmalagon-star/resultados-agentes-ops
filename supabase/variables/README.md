...
| `016_novedades_copia.sql` | Novedades (D-024/D-026): tablas `nov_*` de copia desde SGN, estado de órdenes en Dropi (Broker) e IA, sin datos personales; `var_pais_iso`, `var_estado_final`, `nov_candidatas_estado`; Edge `sync-sgn` (cada 10 min) y `nov-estado-dropi` (05:00–08:55 COL cada 5 min, sondeo secuencial de jobs del Broker) | 2026-09-25 |
| `017_candidatas_sin_tope.sql` | candidatas de estado Dropi sin tope de tiempo (180 días; diario 14 días, luego semanal), tandas de 1.000 sin repetir lo consultado hoy (aplicada como 017+018) | 2026-09-25 |
| `019_novedades_medicion.sql` | Novedades etapa 2: `var_agente.correo` + `var_nov_puente`, metas de Novedades en `var_config_default` (medible=false: el pago no cambia), `var_novedades_mes` (Solución, Gestión Ácida, Gestiones/h, pendientes de cierre), `nov_sin_gestion` (reporte Admin), `var_roster` con correo/enlace (020) | 2026-09-25 |
| `021_novedades_revision.sql` | Correcciones de la revisión adversaria de Novedades: Gestión Ácida solo a la última solución antes del desenlace; reincidencia por Dropi o por eventos SGN; agente-día sin gestiones cuenta horas; domingo fuera del numerador; acumulado por órdenes distintas; `sin_asignar`/`sin_pais`; `var_nov_puente` des-enlaza; `nov_sin_gestion` acotado a cada novedad; índice parcial de eventos cerrados | 2026-09-25 |
| `022_nov_sin_gestion_universo.sql` | Reporte Admin con el universo completo: `universo` (resueltas por la IA antes de llegar al equipo + llegaron al equipo) y `por_dia` con esas dos columnas; índices parciales | 2026-09-25 |
| `023_config_novedades.sql` | Configuración del mes para Novedades: `var_config_vigente` completa las variables de Novedades si la config guardada no las trae; `var_config_guardar` valida pesos (100) y metas (> 0) también en cargos con variables no medibles | 2026-09-25 |
