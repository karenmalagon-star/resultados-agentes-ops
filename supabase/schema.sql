...
-- ============================================================
-- monitor_checks(): chequeo de salud del dashboard (2026-08-26)
-- Lo consume la Edge Function monitor-salud. Solo service_role.
-- ============================================================
...
begin
  ...
  if v_snap is null or v_age > 75 then
    problemas := problemas || jsonb_build_array(
      ...
      ' minutos (última: ' || coalesce(to_char(v_snap at time zone 'America/Bogota', 'YYYY-MM-DD HH24:MI'), 'nunca') || ' hora Colombia).');
  end if;
  for r in
  ...
  loop
    if r.age_min > r.umbral then
      problemas := problemas || jsonb_build_array(
        'El panel "' || r.key || '" no se actualiza hace ' || round(r.age_min) ||
        ' minutos (última: ' || to_char(r.updated_at at time zone 'America/Bogota', 'YYYY-MM-DD HH24:MI') || ' hora Colombia).');
    end if;
  end loop;
  select jsonb_agg(
    'Corrida fallida a las ' || to_char(created at time zone 'America/Bogota', 'HH24:MI') ||
    ' (hora Colombia): HTTP ' || coalesce(status_code::text, '?') || ' — ' ||
    coalesce(left(content::text, 150), coalesce(error_msg, 'sin detalle')))
  into fallas
  from net._http_response
  where created > now() - interval '35 minutes'
    and (status_code is distinct from 200 or error_msg is not null or content::text like '%"ok":false%');
